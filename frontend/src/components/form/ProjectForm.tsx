        ...
        <div className={`nightMode bg-foreground rounded-md p-5 flex flex-col transition-all duration-300 ease-in-out ${
            ...
            <div className="absolute bottom-2 right-5 z-10">
                <button
                    ...
                    aria-label={isExpanded ? "Réduire le formulaire" : "Agrandir le formulaire"}
                >
                    {isExpanded ? <FaCompress size={24}/> : <FaExpand size={24}/>}
                </button>
            </div>
            <div className={`flex flex-col gap-4 ${isExpanded ? 'h-full' : 'lg:flex-row'}`}>
                <div className={`${isExpanded ? 'flex-grow' : 'lg:w-1/3'}`}>
                    <FileInput onChange={files => setImages(files)}/>
                </div>
                <div className={`space-y-4 ${isExpanded ? 'flex-grow flex flex-col' : 'lg:w-2/3'}`}>
                    <Input
                        type="text"
