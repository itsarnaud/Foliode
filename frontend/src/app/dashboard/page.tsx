								...
								<div className="sm:col-span-1 md:col-span-1 lg:col-span-1 xl:col-span-1 flex flex-col gap-4">
									<Card className="w-full shadow-lg hover:shadow-xl transition-shadow duration-300 border-2 border-gray-200 dark:border-[#2C2D33]">
										<CardBody className="flex flex-row items-center gap-5 p-4">
											...
											{user?.github_login ? 
												<span className="dayMode">Connecté en tant que <Link href={`https://github.com/${user.github_login}`} underline="hover" isExternal className="cursor-pointer">{user?.github_login}</Link></span> 
											: 
												<>
													<Link onPress={() => signInGitHub()} isExternal showAnchorIcon underline="hover" className="text-foreground cursor-pointer">
									...
									<Card className="w-full overflow-hidden shadow-lg hover:shadow-xl duration-300 border-2 border-gray-200 dark:border-[#2C2D33]">
										<CardBody className="flex flex-row items-center gap-5 p-4">
											...
											{user?.dribbble_login ? 
												<span className="dayMode">Connecté en tant que <Link href={`https://dribbble.com/${user.dribbble_login}`} underline="hover" isExternal className="cursor-pointer">{user?.dribbble_login}</Link></span> 
											: 
												<>
													<Link onPress={() => signInDribbble()} isExternal showAnchorIcon underline="hover" className="text-foreground cursor-pointer">
									...
									<CardBody>
										...
										{portfolioPromotion ? (
											<div className="space-y-2">
												<p><span className="font-semibold">Type:</span> {portfolioPromotion.formation.type}</p>
												<p><span className="font-semibold">Formation:</span> {portfolioPromotion.formation.name}</p>
											</div>
										) : (
								...
								<Card className="sm:col-span-2 md:col-span-3 lg:col-span-2 xl:col-span-2 shadow-lg hover:shadow-xl duration-300 border-2 border-gray-200 dark:border-[#2C2D33]">
									<CardBody>
										<div className="flex justify-between items-center mb-4">
											...
											<Link href="/dashboard/skills" className="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200">
												<FaPencilAlt className="dayMode text-primary duration-200 hover:text-primary-200 hover:scale-110" />
											</Link>
										</div>
										{portfolio?.tools && portfolio.tools.length > 0 ? (
											<div className="flex flex-wrap gap-2">
												{portfolio.tools.map((tool, index) => (
													<div key={index} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-full text-sm">
														{tool.name}
													</div>
												))}
											</div>
								...
								<Card className="sm:col-span-2 md:col-span-3 lg:col-span-2 xl:col-span-3 shadow-lg hover:shadow-xl duration-300 border-2 border-gray-200 dark:border-[#2C2D33]">
									<CardBody>
										<div className="flex justify-between items-center mb-4">
											...
											<Link href="/dashboard/projects" className="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200">
												<FaPencilAlt className="dayMode text-primary duration-200 hover:text-primary-200 hover:scale-110" />
											</Link>
										</div>
										{projects.length > 0 ? (
											<div className="space-y-3">
												...
												{projects.length > 3 && (
													<p className="text-sm text-gray-500 dark:text-gray-400">+ {projects.length - 3} autres projets</p>
												)}
											</div>
										) : (
								...
								<Card className="sm:col-span-2 md:col-span-3 lg:col-span-2 xl:col-span-2 shadow-lg hover:shadow-xl duration-300 border-2 border-gray-200 dark:border-[#2C2D33]">
									<CardBody>
										<div className="flex justify-between items-center mb-4">
											...
											<Link href="/dashboard/edit" className="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200">
												<FaPencilAlt className="dayMode text-primary duration-200 hover:text-primary-200 hover:scale-110" />
											</Link>
										</div>
										{portfolioColors ? (
										...
										<CardBody>
												...
												<div className="h-48 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-lg">
														<p className="text-gray-500 dark:text-gray-400 text-center">
																...
																<span className="text-sm">Suivez les vues et l&apos;engagement de votre portfolio !</span>
														</p>
												</div>
